...
class BookDetails extends Component{
    ...
    getBookDetailsData=async()=>{
        ...
        if(response.ok){
            ...
            this.setState({bookDetailsData:fetchedData,apiStatus:apiStatusConstants.success});
        }
    ...
    renderBookDetailsView=()=>{
        const {title,subtitle,authors,publisher,language,isbn10,isbn13,pages,year,rating,price,desc,image}=this.state.bookDetailsData;

        const onClickAddToCart=()=>{
            onClickAddToCart({...this.state.bookDetailsData})
        }
                ...
                <div className="book-details-content-container">
                    <div className="book-basic-details-container">
                        ...
                        <div className="book-other-basic-details">
                            <h1 className="book-details-heading book-details-title">{title}</h1>
                            <p className="book-details-subtitle">{subtitle}</p>
                            <p className="book-details-author">{authors}</p>
                            <p className="book-details-release-year">Release Year: {year}</p>
                            <hr className="horizontal-rule-display"/>
                            <p className="book-details-price">{price}</p>
                            <div className="buttons-container">
                                <button className="book-details-button" onClick={onClickAddToCart}>
                                    Add to Cart
                                </button>
                            </div>
                        </div>
                    </div>
                    <hr classNamr="horizontal-rule"/>
                    <div>
